...
const int echoPin = 3;  // Grove Ultrasonic Sensor uses single digital pin

// Button-Pin
const int Taster = 4; // Pin für den Button -> D4
...
void setup() {
  ...
  schranke.write(winkelZu); // Schranke geschlossen
...
void schrankeSteuern() {
  ...
  if (entfernung > 0 && entfernung < schwellwert && !schrankeOffen) {
    schranke.write(winkelAuf); // Schranke öffnet
    schrankeOffen = true;
  ...
  if (schrankeOffen && millis() - schrankeTimer >= schrankeOffenZeit) {
    schranke.write(winkelZu); // Schranke schließt
    schrankeOffen = false;
  }
}
